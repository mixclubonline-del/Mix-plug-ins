...
class PrimeEQVstBridge extends VstBridge<PrimeEQSettings> {
    ...
    constructor(initialSettings: PrimeEQSettings) {
    ...
    public dspProcess(
        ...
        const moodColors: Record<string, { start: string, end: string }> = {
            'Neutral': { start: '#00ffff', end: '#A57CFF' },
            'Warm': { start: '#f59e0b', end: '#ef4444' },
            'Bright': { start: '#06b6d4', end: '#fcd34d' },
            'Dark': { start: '#6366f1', end: '#1e1b4b' },
            'Energetic': { start: '#ec4899', end: '#8b5cf6' },
        };
        const colors = moodColors[mood as string] || moodColors['Neutral'];
        ...
        return {
            ...
            spectrumColorStart: colors.start,
            spectrumColorEnd: colors.end,
        };
    }
}
...
({ visualizerData, focus, width, height }) => {
    ...
    const { spectrum, pathData, focusX, focusY, spectrumColorStart, spectrumColorEnd } = visualizerData;
        ...
        <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMidYMid meet">
            <defs>
                <linearGradient id="prime-eq-grad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor={spectrumColorStart} />
                    <stop offset="100%" stopColor={spectrumColorEnd} />
                </linearGradient>
                <linearGradient id="prime-eq-fill" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor={spectrumColorStart} stopOpacity="0.3" />
                    <stop offset="100%" stopColor={spectrumColorEnd} stopOpacity="0" />
                </linearGradient>
