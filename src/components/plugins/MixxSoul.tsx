...
class MixxSoulVstBridge extends VstBridge<MixxSoulSettings> {
    constructor(initialSettings: MixxSoulSettings) {
    ...
    public dspProcess(
        ...
        const moodHueMap: Record<string, number> = {
            'Warm': 30, 'Bright': 60, 'Dark': 270, 'Energetic': 330, 'Neutral': 210 
        };
        const baseHue = moodHueMap[mood as string] || 210;

        const targetHue = 50;
        const hue = baseHue + (targetHue - baseHue) * normalizedEmpathy * 0.5; // Reduced influence of empathy on hue shift to keep mood color dominant
